...
async function generator() {
  ...
  const mustacheData = {
    ...
    attachLibrary: `{{ attach_library('guswds/${componentName}') }}`,
    // Lambdas
  ...
  if (shouldCreate) {
    ...
    if (library) {
      const root = path.resolve(__dirname, '..');
      const librariesFile = path.join(root, 'guswds.libraries.yml');
      try {
        ...
        if (modularSass || componentJs) {
          newLibrary.dependencies = ['guswds/global'];
        }
        ...
        console.error(
          `${librariesFile} could not be opened. Please ensure the file exists. If your theme is not called guswds, update the component.js script to look for the correct file name.`
        );
      }
    }
