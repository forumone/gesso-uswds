...
const { UswdsValue } = require('./UswdsValue.cjs');
...
const paletteTransformer = (node, doc, map) => {
  const value =
    node instanceof SassValue || node instanceof UswdsValue ? node : node.value;
...
const fontFamilyTransformer = (node, doc, map) => {
  ...
  if (valueByKey === undefined) {
    if (node.range) {
      ...
      throw new NaniError(`Could not resolve '${key}' in gesso.typography.font-family`);
    }
  }

  if (valueByKey instanceof SassValue || valueByKey instanceof UswdsValue) {
    return valueByKey;
  }
...
const uswdsTransformer = (node, doc, map) => {
  ...
  if (value instanceof UswdsValue) {
    value.setToken(
      uswdsTransformer(doc.createNode(value.getToken()), doc, map)
    );
    return value;
  }
  const key = String(value);
  const keys = key.split('.');
  const valueByKey = doc.getIn(keys, true);
  if (valueByKey !== undefined) {
    return identityTransformer(valueByKey, doc, map);
  }
