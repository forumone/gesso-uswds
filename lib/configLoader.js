...
module.exports = function (source) {
  ...
  try {
    readSource(source)
      .then(transform)
      .then(transformed => {
        const uswds_sass = uswdsSassComment + os.EOL + renderUswdsTheme(transformed.data);
        const sass = sassComment + os.EOL + "@use '_uswds-theme-settings.artifact.scss' as *;" + os.EOL + renderSass(transformed.data);
        const js = jsComment + os.EOL + renderJs(transformed.data);
        this.emitFile('../source/00-config/_uswds-theme-settings.artifact.scss', uswds_sass);
