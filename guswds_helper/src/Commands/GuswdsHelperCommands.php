...
namespace Drupal\guswds_helper\Commands;
 ...
 */
class GuswdsHelperCommands extends DrushCommands implements SiteAliasManagerAwareInterface {
  ...
  /**
   * Create a new theme based on the Gesso USWDS theme.
   *
   * @param string $name
   ...
   * @usage drush guswds "New Theme Name"
   *   Creates a new theme called "New Theme Name" with a machine name
   *   of "new_theme_name".
   * @usage drush guswds "New Theme Name" --machine-name=new_theme
   *   Creates a new theme called "New Theme Name" with a machine name
   *   of "new_theme".
   *
   * @command guswds
   ...
   * @throws \Exception
   */
  public function guswds($name, array $options = ['description' => NULL, 'machine-name' => NULL]) {
    // Get new theme options.
    if (!isset($name)) {
      $name = $options['name'];
    }
    $machine_name = $options['machine-name'] ?: $this->guswdsMachineName($name);
    $description = $options['description'];

    // Validate the command.
    if (!$this->guswdsThemeExists('guswds')) {
      throw new \Exception(dt('Where is the Gesso USWDS theme? I could not find it.'));
    }

    if ($this->guswdsThemeExists($machine_name)) {
    ...
    $guswds_path = Path::join($drupalRoot, drupal_get_path('theme', 'guswds'));
    $theme_path = substr($guswds_path, 0, strrpos($guswds_path, '/'));
    $new_path = Path::join($theme_path, $machine_name);

    // Copy the Gesso USWDS theme directory recursively to the new theme’s
    // location.
    $fs = new Filesystem();
    $fs->mirror($guswds_path, $new_path);

    // Remove Gesso USWDS’s helper module from the new theme.
    $this->guswdsRecursiveRm(Path::join($new_path, 'guswds_helper'));

    // Rename the .info.yml file.
    $guswds_info_file = Path::join($new_path, 'guswds.info.yml');
    $new_info_file = Path::join($new_path, $machine_name . '.info.yml');
    drush_op('rename', $guswds_info_file, $new_info_file);
    ...
    $this->guswdsFileStrReplace($new_info_file, array_keys($changes), $changes);

    // Rename the .breakpoints.yml file.
    $guswds_info_file = Path::join($new_path, 'guswds.breakpoints.yml');
    $new_info_file = Path::join($new_path, $machine_name . '.breakpoints.yml');
    drush_op('rename', $guswds_info_file, $new_info_file);

    // Rename the .libraries.yml file.
    $guswds_libraries_file = Path::join($new_path, 'guswds.libraries.yml');
    $new_libraries_file = Path::join($new_path, $machine_name . '.libraries.yml');
    drush_op('rename', $guswds_libraries_file, $new_libraries_file);

    // Rename the .layouts.yml file.
    $guswds_layouts_file = Path::join($new_path, 'guswds.layouts.yml');
    $new_layouts_file = Path::join($new_path, $machine_name . '.layouts.yml');
    drush_op('rename', $guswds_layouts_file, $new_layouts_file);

    // Rename the .theme file.
    $guswds_theme_file = Path::join($new_path, 'guswds.theme');
    $new_theme_file = Path::join($new_path, $machine_name . '.theme');
    drush_op('rename', $guswds_theme_file, $new_theme_file);

    // Replace all occurrences of 'guswds'
    ...
    foreach ($files as $file) {
      $this->guswdsFileStrReplace(Path::join($new_path, $file), ['guswds', 'Gesso USWDS'], [$machine_name, $name]);
    }
    ...
    $this->io()->caution(dt('If you want to remove the Gesso USWDS theme entirely, be sure to copy and rename the '
     . 'guswds_helper module first.'));
  }

  /**
   * Converts $name to a machine-readable format.
   */
  private function guswdsMachineName($name) {
   ...
   */
  private function guswdsThemeExists($theme_name) {
   ...
   */
  private function guswdsFileStrReplace($file_path, $find, $replace) {
   ...
   */
  private function guswdsRecursiveRm($path) {
    if (is_dir($path)) {
      ...
      foreach ($dir_contents as $item) {
        if ($item !== '.' && $item !== '..') {
          ...
          if (is_dir($subpath)) {
            $this->guswdsRecursiveRm($subpath);
          }
          else {
