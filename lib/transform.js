...
const paletteTransformer = (node, doc, map) => {
  ...
  const value = /** @type {import('./types').GessoScalar | undefined } */ (
    doc.getIn(keys)
  );
  if (value === undefined) {
    if (node.range) {
...
const uswdsTransformer = (node, doc, map) => {
  ...
  if (node instanceof UswdsValue) {
    node.setValue(uswdsTransformer(doc.createNode(node.getValue()), doc, map));
    return node;
  }
  const key = String(node.value);
