...
The ESLint config can be changed in the `.eslintrc.cjs` file. Gesso USWDS follows the
...
1. Add jQuery to `externals` in lines 173-177 of `webpack.common.js`
   ```js
   externals: {
      drupal: 'Drupal',
      drupalSettings: 'drupalSettings',
   ...
   ```
2. Ensure that `core/jquery` is added a dependency of the appropriate library in guswds.libraries.yml
   ```yaml
   library_name:
     js:
